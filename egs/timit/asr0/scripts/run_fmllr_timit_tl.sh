...
dnn_pref=dnn4_pretrain-dbn_dnn_smbr
...
if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
    ...
    utils/combine_data.sh --extra_files 'utt2num_frames tokenid.scp monoid.scp' data-fmllr-tri3/${train_set}_org data-fmllr-tri3/${train_set}
    utils/combine_data.sh --extra_files 'utt2num_frames tokenid.scp monoid.scp' data-fmllr-tri3/${train_dev}_org data-fmllr-tri3/${train_dev}
...
if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
    ...
    for part in ${train_set} ${train_dev}; do
        local/data2json.sh --idscp "monoid.scp" --feat ${dumpdir}/${part}/delta${do_delta}/feats.scp \
            data-fmllr-tri3/${part} > ${dumpdir}/${part}/delta${do_delta}/data_mono_${bpemode}${nbpe}.json
    done
...
if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --train-mono-json ${feat_tr_dir}/data_mono_${bpemode}${nbpe}.json \
...
if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
    ...
    (
        ...
        # get decoded results
        local/decode_dnn.sh --num-threads $nthreads exp/tri3/graph exp/${dnn_pref}_ali_${rtask} ${feat_recog_dir} ${expdir}/${decode_dir} || exit 1;
        local/score.sh --min-lmwt 4 --max-lmwt 23 data-fmllr-tri3/${rtask} exp/tri3/graph ${expdir}/${decode_dir} || exit 1;
