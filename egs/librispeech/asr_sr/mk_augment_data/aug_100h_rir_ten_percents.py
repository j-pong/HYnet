...
if not os.path.isdir("./data/fairseq/train_aug100_10_percents"):
    os.makedirs("./data/fairseq/train_aug100_10_percents")

with open("./data/fairseq/train_clean_100/train.tsv" , "r") as f:
    root_dir = f.readline().strip()
    save_root_dir = "/DB/LibriSpeech/LibriSpeech/train_aug100_10_percents"

    with open("./data/fairseq/train_aug100_10_percents/train.tsv", 'w') as aug_f: 
        ...
        for i, line in enumerate(f):
            ...
            if not aug == 1 :    
                ...
                sf.write("/DB/LibriSpeech/LibriSpeech/train_aug100_10_percents/" + items[0][:index] + "aug" +items[0][index:], aug_wav, curr_sample_rate)
                aug_f.write(items[0][:index] + "aug" +items[0][index:]+"\t"+items[1] +"\n")
            else:
                sf.write("/DB/LibriSpeech/LibriSpeech/train_aug100_10_percents/" + items[0][:index] +items[0][index:], wav, curr_sample_rate)
