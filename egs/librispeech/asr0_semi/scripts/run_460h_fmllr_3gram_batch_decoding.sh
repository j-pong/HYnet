...
ncore=28       # number of cores for calculating number of threads
nj=32
dumpdir=dump
resume=
unsup_resume=
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    for part in dev_clean test_clean dev_other test_other train_clean_100 train_clean_360; do
...
if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
    ...
    # decode using the tri4b model with pronunciation and silence probabilities
    {
        ...
        for test in train_clean_360 dev_clean dev_other test_clean test_other; do
...
feat_label_tr_dir=${dumpdir}/train_100/delta${do_delta}; mkdir -p ${feat_label_tr_dir}
feat_unlabel_tr_dir=${dumpdir}/train_360/delta${do_delta}; mkdir -p ${feat_unlabel_tr_dir}
...
if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
    ...
    for x in train_clean_100 train_clean_360 dev_clean dev_other test_clean test_other; do
    ...
    utils/combine_data.sh --extra_files 'utt2num_frames tokenid.scp' data/train_360_org data/train_clean_360_fmllr
    ...
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta ${do_delta} \
        data/train_100/feats.scp data/train_360/cmvn.ark exp/dump_feats/train ${feat_label_tr_dir}
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta ${do_delta} \
        data/train_360/feats.scp data/train_360/cmvn.ark exp/dump_feats/train ${feat_unlabel_tr_dir}
    dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta ${do_delta} \
        data/${train_dev}/feats.scp data/train_360/cmvn.ark exp/dump_feats/dev ${feat_dt_dir}

    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}; mkdir -p ${feat_recog_dir}
        dump.sh --cmd "$train_cmd" --nj ${nj} --do_delta ${do_delta} \
            data/${rtask}_fmllr/feats.scp data/train_360/cmvn.ark exp/dump_feats/recog/${rtask} \
            ${feat_recog_dir}
    done
fi
...
if [ -z ${tag} ]; then
    expname=train_460_$(basename ${train_config%.*})
    if ${do_delta}; then
...
if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --verbose 0 \
        --resume ${resume} \
        --label-train-json ${feat_label_tr_dir}/data_${bpemode}${nbpe}.json \
        --unlabel-train-json ${feat_unlabel_tr_dir}/data_${bpemode}${nbpe}.json \
