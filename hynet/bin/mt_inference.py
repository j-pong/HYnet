...
class Text2Text:
    ...
    def __init__(
        self,
        mt_train_config: Union[Path, str],
    ...
    @torch.no_grad()
    def __call__(
        self, text_input: Union[torch.Tensor, np.ndarray]
        ...
        text_input = text_input.unsqueeze(0).to(getattr(torch, self.dtype))
        # lenghts: (1,)
        lengths = text_input.new_full([1], dtype=torch.long, fill_value=text_input.size(1))
        batch = {"text_input": text_input, "text_input_lengths": lengths}
...
def get_parser():
    ...
    group.add_argument("--mt_train_config", type=str, required=True)
    group.add_argument("--mt_model_file", type=str, required=True)
    ...
    group.add_argument(
        ...
        help="The token type for MT model. "
