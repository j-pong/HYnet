...
class VGGRNNEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        self.pad = 0
        self.embed = torch.nn.Embedding(input_size, hidden_size, padding_idx=self.pad)
        ...
        if use_projection:
            self.enc = torch.nn.ModuleList(
                [
                    VGG2L(in_channel),
                    RNNP(
                        get_vgg2l_odim(hidden_size, in_channel=in_channel),
                        num_layers,
                        hidden_size,
                        output_size,
                        subsample,
    ...
    def forward(
        ...
        # FIXME(jpong): where padding for batchfy occurs? why padding idx is -1(ignore_id maybe?)?
        if -1 in xs_pad:
            xs_pad[xs_pad == -1] = self.pad
        xs_pad = self.dropout_emb(self.embed(xs_pad))
