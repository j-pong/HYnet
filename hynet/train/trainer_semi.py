...
class PseudoTrainer(Trainer):
    ...
    def run(
        ...
        for iepoch in range(start_epoch, max_epoch + 1):
            ...
            # Set pretrain mode
            pretrain = False
            if pretrain_step is not None and int(pretrain_step) > int(iepoch):
                pretrain = True
            # 1. Train and validation for one-epoch
            with reporter.observe("train") as sub_reporter:
                all_steps_are_invalid = cls.train_one_epoch(
            ...
            if int(iepoch) < 60:
                model.th_beta = model.th_beta.new([float(((iepoch-20) / 20) * 0.025)])
            else:
                model.th_beta = model.th_beta.new([0.05])
            # model.th_beta = model.th_beta.new([0.6])
        ...
        for iiter, (_, batch) in enumerate(
            ...
            with autocast(scaler is not None):
                with reporter.measure_time("forward_time"):
                    loss, stats, weight = model(**batch, pretrain=pretrain)
                    if not pretrain:
                        loss_pseudo, stats_pseudo, weight_pseudo = model(**pseudo_batch, mode='pseudo')

                stats_temp = {}
                for k, v in stats.items():
                    if v is not None and k is not "pretrain" and k is not "err_pred":
                        stats_temp[k] = (v + stats_pseudo[k]) / 2
                    elif v is not None and k is "pretrain":
                        stats_temp[k] = v
