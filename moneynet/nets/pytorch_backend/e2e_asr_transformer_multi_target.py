...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, mono_odim, args, ignore_id=-1):
        ...
        self.poster_mono = torch.nn.Linear(args.adim, mono_odim * self.oversampling)
        if self.outer:
            if self.residual:
    ...
    def target_match(self, xs_pad, hs_pad, ys_pad, poster):
        if self.outer:
            post_pad = poster(hs_pad)
    ...
    def forward(self, xs_pad, ilens, ys_pad, ys_pad_mono=None):
        ...
        pred_pad, ys_pad = self.target_match(xs_pad, hs_pad, ys_pad, self.poster)
        self.pred_pad = pred_pad
        pred_pad_mono, ys_pad_mono = self.target_match(xs_pad, hs_pad, ys_pad, self.poster_mono)
