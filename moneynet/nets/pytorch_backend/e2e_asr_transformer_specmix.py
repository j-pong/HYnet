...
from moneynet.transform.specaugment_mixup import Specmix
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        # augmentation
        group.add_argument(
            "--data-augmentation", default="specmix", type=str, help="augmentation category"
        )
        group.add_argument(
            "--mixup-alpha", default=0.5, type=float, help="mixup alpha between 0~1"
        )
        group.add_argument(
            "--mixup-scheme", default="global", type=str, choices=["local", "global"], help="mixup scheme"
        )
        group.add_argument(
            "--Fmask", default="15", type=int, help="Frequency mask range"
        )
        group.add_argument(
            "--F-n-mask", default="2", type=int, help="# of frequency mask"
        )
        group.add_argument(
            "--Tmask", default="40", type=int, help="Time mask range"
        )
        group.add_argument(
            "--T-n-mask", default="2", type=int, help="# of time mask"
        )
        return parser
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        self.spec_mixup = dict()
        self.spec_mixup["augmentation"] = args.data_augmentation
        self.spec_mixup["mixup_alpha"] = args.mixup_alpha
        self.spec_mixup["mixup_scheme"] = args.mixup_scheme
        self.spec_mixup["F"] = args.Fmask
        self.spec_mixup["F_n_mask"] = args.F_n_mask
        self.spec_mixup["T"] = args.Tmask
        self.spec_mixup["T_n_mask"] = args.T_n_mask
    ...
    def forward(self, xs_pad, ilens, ys_pad, train=True):
        """E2E forward.
        ...
        # apply SpecMix
        xs_pad, ilens, ys_pad, ys_pad_b, lam, src_mask = Specmix(xs_pad, ilens, ys_pad, src_mask, self.spec_mixup, train=train)

        if xs_pad.size(1) != ys_pad_b.size(1):
            if xs_pad.size(1) < ys_pad_b.size(1):
                ys_pad_b = ys_pad_b[:, :xs_pad.size(1)].contiguous()
            else:
                raise ValueError("target size {} is smaller than input size {}".format(ys_pad.size(1), xs_pad.size(1)))
        ...
        if pred_pad.size(1) != ys_pad_b.size(1):
            if pred_pad.size(1) < ys_pad_b.size(1):
                ys_pad_b = ys_pad_b[:, :pred_pad.size(1)].contiguous()
            else:
                raise ValueError("target size {} and pred size {} is mismatch".format(ys_pad.size(1), pred_pad.size(1)))
