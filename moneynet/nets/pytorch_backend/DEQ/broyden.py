...
def broyden(g, x0, threshold, eps, ls=False, name="unknown"):
    ...
    bsz, seq_len, total_hsize = x0.size()
    ...
    Us = torch.zeros(bsz, seq_len, total_hsize, threshold).to(x0.device)  # One can also use an L-BFGS scheme to further reduce memory
    VTs = torch.zeros(bsz, threshold, seq_len, total_hsize).to(x0.device)
