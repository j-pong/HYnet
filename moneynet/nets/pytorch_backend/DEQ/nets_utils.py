...
class RootFind(Function):
    ...
    @staticmethod
    def f(func, xs_pad, ilens, prev_state):
        return func(xs_pad, ilens, prev_state=prev_state)

    @staticmethod
    def g(func, xs_pad, ilens, prev_state):
        return RootFind.f(func, xs_pad, ilens, prev_state) - xs_pad

    @staticmethod
    def broyden_find_root(func, xs_pad, ilens, prev_state, eps, *args):
        ...
        g = lambda x: RootFind.g(func, x, ilens, prev_state)
    ...
    @staticmethod
    def forward(ctx, func, xs_pad, ilens, prev_state, *args):
        batch, seq_len, cdim = xs_pad.size()
        eps = 1e-6 * np.sqrt(batch * seq_len * cdim)
        root_find = RootFind.broyden_find_root
        ctx.args_len = len(args)
        with torch.no_grad():
            z1ss_est = root_find(func, xs_pad, ilens, prev_state, eps, *args)  # args include pos_emb, threshold, train_step
...
class DEQModule(nn.Module):
    ...
    class Backward(Function):
        ...
        @staticmethod
        def forward(ctx, func_copy, z1ss, ilens, prev_state, *args):
            ctx.save_for_backward(z1ss, ilens, prev_state)
            ctx.func = func_copy
            ctx.args = args
            return z1ss
        ...
        def backward(ctx, grad):
            ...
            grad = grad.clone()
            z1ss, ilens, prev_state = ctx.saved_tensors
            args = ctx.args
            ...
            with torch.enable_grad():
                y = RootFind.g(func, z1ss, ilens, prev_state)
