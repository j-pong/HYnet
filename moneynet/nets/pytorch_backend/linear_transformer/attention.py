...
class MultiHeadedAttention(nn.Module):
    ...
    def forward_attention(self, value, scores, mask):
        ...
        x = (
            value.transpose(1, 2).contiguous().view(n_batch, -1, self.h * self.d_k)
        )  # (batch, time1, d_model)
    ...
    def forward(self, query, key, value, mask):
        ...
        # scores = torch.matmul(F.elu(q), F.elu(k.transpose(-2, -1)))
        return self.forward_attention(v, None, mask)
