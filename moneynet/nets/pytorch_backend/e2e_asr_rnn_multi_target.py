...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, ys_pad, ys_pad_mono=None):
        ...
        if ys_pad_mono is not None:
            ...
            pred_pad_mono = pred_pad_mono.view(pred_pad_mono.size(0), -1, self.mono_odim)
            self.pred_pad_mono = pred_pad_mono
            if pred_pad_mono.size(1) != ys_pad_mono.size(1):
                if pred_pad_mono.size(1) < ys_pad_mono.size(1):
                    ys_pad_mono = ys_pad_mono[:, :pred_pad_mono.size(1)].contiguous()
                else:
                    raise ValueError("target size {} and pred size {} is mismatch".format(ys_pad_mono.size(1), pred_pad_mono.size(1)))
