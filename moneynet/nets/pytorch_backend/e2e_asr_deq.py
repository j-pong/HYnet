...
# from moneynet.nets.pytorch_backend.DEQ.nets_utils import SplitCrossEntropyLoss
from moneynet.nets.pytorch_backend.DEQ.encoders import encoder_for
...
class Reporter(chainer.Chain):
    ...
    def report(self, loss_ce, acc, mtl_loss):
...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        # self.criterion = SplitCrossEntropyLoss(args.eprojs, splits=cutoffs, verbose=False)
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        # 1. RNN Encoder
        hs_pad, new_mem = self.enc(hs_pad, hlens)
        ...
        if LooseVersion(torch.__version__) < LooseVersion("1.0"):
            ...
            reduction_str = "mean"
        self.loss_ce = self.loss = F.cross_entropy(
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(
                loss_ce_data, self.acc, loss_data
            )
        else:
            pass
        return self.loss
