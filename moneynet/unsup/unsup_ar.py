...
class Reporter(object):
    ...
    def report_plot_buffer(self, keys, epoch, tag=None, merge=False):
        for key in keys:
        ...
        if merge:
            ...
            for i, key in enumerate(keys):
                if tag is not None:
                    ...
                    if i == 0:
                        filename = '{}.png'.format(key)
                ax.plot(self.report_buffer[key], label=key)
            ax.grid()
            ax.legend()
...
def train(args):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if (epoch + 1) % args.high_interval_epochs == 0:
            ...
            reporter.report_image(keys=['x_ele0', 'x_ele1', 'x_ele2', 'x_ele3', 'x_ele4'], filename=filename)
            ...
            reporter.report_plot_buffer(keys=['loss', 'loss_x', 'loss_y'], epoch=epoch + 1, tag='_eval', merge=True)
        if (epoch + 1) % args.low_interval_epochs == 0:
            reporter.report_plot_buffer(keys=['loss', 'loss_x', 'loss_y'], epoch=epoch + 1, merge=True)
