...
from moneynet.asr.pytorch_backend.asr_init import load_trained_multi_target_model
from moneynet.asr.pytorch_backend.asr_init import load_trained_multi_target_modules
            ...
            for batch in it:
                ...
                with reporter_module.report_scope(observation):
                    ...
                    if self.ngpu == 0:
                        ...
                        data_parallel(self.model, (*x,None), range(self.ngpu))
...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        train_mono_iter = self.get_iterator("sub")
        optimizer = self.get_optimizer("main")
        epoch = train_iter.epoch

        # Get the next batch (a list of json files)
        batch = train_iter.next()
        batch_mono = train_mono_iter.next()
        # self.iteration += 1 # Increase may result in early report,
        # which is done in other place automatically.
        x = _recursive_to(batch, self.device)
        x_mono = _recursive_to(batch_mono, self.device)
        x_mono_target = x_mono[-1]
        ...
        if self.ngpu == 0:
            loss = self.model(*x,x_mono_target).mean() / self.accum_grad
        else:
            # apex does not support torch.nn.DataParallel
            loss = (
                    data_parallel(self.model, (*x,x_mono_target), range(self.ngpu)).mean() / self.accum_grad
            )
        if self.use_apex:
...
def train(args):
    ...
    with open(args.train_mono_json, "rb") as f:
        train_mono_json = json.load(f)["utts"]
    utts = list(valid_json.keys())
    mono_utts = list(train_mono_json.keys())
    idim_list = [
        int(valid_json[utts[0]]["input"][i]["shape"][-1]) for i in range(args.num_encs)
    ]
    odim = int(valid_json[utts[0]]["output"][0]["shape"][-1])
    mono_odim = int(train_mono_json[mono_utts[0]]["output"][0]["shape"][-1])
    for i in range(args.num_encs):
        logging.info("stream{}: input dims : {}".format(i + 1, idim_list[i]))
    logging.info("#output dims: " + str(odim) + ", " + str(mono_odim))
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        model = load_trained_multi_target_modules(idim_list[0], odim, mono_odim, args)
    else:
        model_class = dynamic_import(args.model_module)
        model = model_class(
            idim_list[0] if args.num_encs == 1 else idim_list, odim, mono_odim, args
        )
    ...
    load_tr_mono = LoadInputsAndTargets(
        mode="asr",
        load_output=True,
        preprocess_conf=args.preprocess_conf,
        preprocess_args={"train": True},  # Switch the mode of preprocessing
    ...
    train_mono_iter = ChainerDataLoader(
        dataset=TransformDataset(train, lambda data: converter([load_tr(data)])),
        batch_size=1,
        num_workers=args.n_iter_processes,
        shuffle=False,
    ...
    updater = CustomUpdater(
        model,
        args.grad_clip,
        {"main": train_iter, "sub": train_mono_iter},
        optimizer,
        device,
        args.ngpu,
        args.grad_noise,
...
def recog(args):
    ...
    """
    set_deterministic_pytorch(args)
    model, train_args = load_trained_multi_target_model(args.model)
    assert isinstance(model, ASRInterface)
