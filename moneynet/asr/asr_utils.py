...
def _rmsprop_lr_decay(trainer, lr_decay):
    optimizer = trainer.updater.get_optimizer("main")
    # for chainer
    if hasattr(optimizer, "lr"):
        current_lr = optimizer.lr
        setattr(optimizer, "lr", current_lr * lr_decay)
        logging.info("rmsprop lr decayed to " + str(optimizer.lr))
    # pytorch
    else:
        for p in optimizer.param_groups:
            p["lr"] *= lr_decay
            logging.info("rmsprop lr decayed to " + str(p["lr"]))


def rmsprop_lr_decay(lr_decay):
    ...
    @training.make_extension(trigger=(1, "epoch"))
    def rmsprop_lr_decay(trainer):
        _rmsprop_lr_decay(trainer, lr_decay)

    return rmsprop_lr_decay
