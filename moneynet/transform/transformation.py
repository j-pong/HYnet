...
import_alias = dict(
    ...
    time_warp="moneynet.transform.spec_augment:TimeWarp",
    time_mask="moneynet.transform.spec_augment:TimeMask",
    freq_mask="moneynet.transform.spec_augment:FreqMask",
    spec_augment="moneynet.transform.spec_augment:SpecAugment",
                    ...
                    if "replace_with" in self.opts[idx]:
                        ...
                        if rep_opt in ["mix", "rmix", "insertion"]:
                            for i, x in enumerate(xs):
                                ...
                                if len(x_mix) > len(x):
                                    x_mix = x_mix[:len(x)]
                                elif len(x_mix) < len(x):
                                    x_mix = numpy.pad(x_mix, ((0, len(x) - len(x_mix)), (0, 0)), 'constant')
                                xs[i] = func(numpy.array([x, x_mix]), **_kwargs)
                        else:
                            xs = [func(x, **_kwargs) for x in xs]
                    elif uttid_list is not None and "uttid" in param:
