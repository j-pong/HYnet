...
from moneynet.transform.functional import FuncTrans
...
def time_warp(x, max_time_warp=80, inplace=False, replace_with="mean", mode="PIL"):
    ...
    """
    x, x_mix = check_mix(x, replace_with)
...
def freq_mask(x, F=30, n_mask=2, replace_with="mean", inplace=False):
...
def time_mask(spec, T=40, n_mask=2, replace_with="mean", inplace=False):
    """freq mask for spec agument
    ...
    :param bool replace_with_zero: pad zero on mask if true else use mean
    """
    spec, x_mix = check_mix(spec, replace_with)
...
def spec_augment(
    ...
    :param bool replace_with_zero: pad zero on mask if true else use mean
    """
    x = time_warp(x, max_time_warp, replace_with=replace_with, inplace=inplace, mode=resize_mode)
    x = freq_mask(
        x,
        max_freq_width,
        n_freq_mask,
